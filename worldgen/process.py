...
from worldgen.diamond_square import diamond_square
from worldgen.diamond_square import seed_corners
...
def process(world_map):
    ...
    diamond_square(smoothness, variance=variance)
    scaling.normalize(smoothness)
    for key in world_maps.ds_generated:
        print("-Processing ", key)
        layer = world_map[key]
        seed_corners(layer)
        diamond_square(layer, smoothing_array=world_map['smoothness'], variance=variance)
